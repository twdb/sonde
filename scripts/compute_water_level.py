...
write_file = raw_input("write output file? [yes/no]: ")
...
water_depth_detection_limit = 0.03
...
sonde_baro_series.ix[:,'corrected_sonde_depth'].dropna().plot(style='b.',
    label='deployed sonde', ax=sonde_depth_ax)    
...
site_deployment_data[site_deployment_data.columns[-3]].dropna().plot(style='r.',
                                                             markersize=12)
site_deployment_data['spotcheck_sensor_depth'].dropna().plot(style='r.',
    markersize=10, label='spot-check sonde', ax=sonde_depth_ax)

sonde_depth_ax.set_title(site_name)
#sonde_depth_ax.set_ylim(0, 2)
sonde_depth_ax.set_ylabel('water depth above sensor, m')
sonde_depth_ax.legend(loc='best')
...
site_deployment_data.total_depth.plot(style='r.', markersize=12,ax=total_depth_ax, 
                                      label= 'spot-check')
plt.legend(loc='best')
plt.savefig(os.path.join(sonde_site_dir, site_name + '_water_depth.png'))


site_deployment_data.total_depth = site_deployment_data.total_depth.apply(
                                    lambda d: np.float(d))
...
sonde_baro_series.ix
plt.ylim(-2,2)
plt.ylabel('water surface elevation,m from NAVD88') 
'''
plt.figure()
sonde_baro_series.water_surface_elevation_med[sonde_baro_series.water_surface_elevation_med < -800.] = np.nan
sonde_baro_series.water_surface_elevation_med.dropna().plot(style='.',
                                                      title=site_name)  
plt.ylabel('water surface elevation,m from NAVD88')
...
plt.title(site_name)
plt.grid(b=None)
plt.savefig(os.path.join(sonde_site_dir, site_name + '_wse.png'))
#plt.xlim(sonde_baro_series.water_surface_elevation_med.index[0],
#         sonde_baro_series.water_surface_elevation_med.index[-1])
"""
...
if write_file == 'yes':
    ...
    disclaimer_header += 'fill_value for bad/missing data: -999.99. \n'
    try: 
    ...
    columnorder = ['raw_pressure_reading', 'air_pressure', 'corrected_sonde_depth',
               ...
               'water_surface_elevation_med', 'water_temperature', 'sonde_id',
               'file_name']
#    units = 'mm/dd/yyyy hh:mm,mH2O,mH2O,mH2O,mH2O,m from NAVD88,m from NAVD88,\
#m from NAVD88,m from NAVD88,degC,n/a,n/a,n/a \n'
...
#    fid.write(units)
    sonde_baro_series.to_csv(fid, sep=',',
                             index_label='datetimes', na_rep = -999.99,
                             float_format='%5.2f')
    fid.close()
